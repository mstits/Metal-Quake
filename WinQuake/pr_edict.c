...
char *pr_strings;
int pr_stringssize;
static char *pr_newstrings;
static int pr_newstringssize;

// Helper to get string from offset, handling extended strings
char *PR_GetString(int offset) {
  if (!pr_strings)
    return "<pr_strings NULL>";
  if (offset >= 0 && offset < pr_stringssize)
    return pr_strings + offset;
  else if (pr_newstrings && offset >= pr_stringssize) {
    if ((offset - pr_stringssize) > 0x1000000) // 16MB sanity limit
      return "<bad offset>";
    return pr_newstrings + (offset - pr_stringssize);
  }
  return "";
}

void PR_ResetNewStrings(void) {
  pr_newstrings = NULL;
  pr_newstringssize = 0;
}
...
string_t ED_NewString(char *string) {
  ...
  // For 64-bit: store pointer in newstrings table and return offset
  // Use negative offsets as marker for extended strings
  if (pr_newstrings == NULL) {
    pr_newstrings = new;
    pr_newstringssize = l;
    ofs = pr_stringssize; // Start at end of original strings
  } else {
    ofs = pr_stringssize + (new - pr_newstrings);
  }
  return ofs;
}
