...
void R_MarkLeaves (void)
{
	...
	// GCD parallel leaf marking — dispatch across P-cores
	extern void MQ_ParallelMarkLeaves(unsigned char *vis, int numleafs,
	                                   void *leaves, int visframecount);
	MQ_ParallelMarkLeaves(vis, cl.worldmodel->numleafs,
	                      cl.worldmodel->leafs, r_visframecount);
}
...
void R_RenderView (void)
{
	...
	// Bypassing software renderer for Metal RT if enabled
	...
	if (vid_rtx.value) {
		...
		// These show through the compositor as non-255 pixels over the RT world
		{
			...
			extern vec3_t r_entorigin, modelorg;
			for (int si = 0; si < cl_numvisedicts; si++) {
				currententity = cl_visedicts[si];
		...
		// Set contents color for underwater tint (normally done in R_RenderView_)
