...
{
	Draw_Pic (x, y + (vid.height-24), pic);
}
...
{
	Draw_TransPic (x, y + (vid.height-24), pic);
}
...
{
	Draw_Character ( x + 4 , y + vid.height-24, num);
}
...
{
	Draw_String (x, y + (vid.height-24), str);
}
...
void Sbar_Draw (void)
{
	...
	if (!vid_rtx.value && sb_updates >= vid.numpages)
		return;
