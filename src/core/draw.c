...
void Draw_Pic (int x, int y, qpic_t *pic)
{
	...
	{
		Sys_Error ("Draw_Pic: bad coordinates (%d, %d) for %dx%d", x, y, pic->width, pic->height);
	}
...
// hack the version number directly into the pic
	...
	sprintf (ver, "%4.2f (Metal)", VERSION);
...
void Draw_TileClear (int x, int y, int w, int h)
{
	...
	if (r_pixbytes == 1)
	{
		for (v=0 ; v<h ; v++)
		{
			dest = vid.buffer + (y + v) * vid.rowbytes + x;
			for (u=0 ; u<w ; u++)
				dest[u] = draw_backtile->data[ ((y+v)&63)*64 + ((x+u)&63) ];
		}
	}
	else if (r_pixbytes == 2)
	{
		...
		for (v=0 ; v<h ; v++)
		{
			pusdest = (unsigned short *)vid.buffer + (y + v) * (vid.rowbytes>>1) + x;
			for (u=0 ; u<w ; u++)
				pusdest[u] = d_8to16table[ draw_backtile->data[ ((y+v)&63)*64 + ((x+u)&63) ] ];
		}
	}
}
